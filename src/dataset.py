...
class GPTDataset(Dataset):
    ...
    def __init__(
        ...
        self._length = (len(self.token_ids) - self.context_length - 1) // self.stride + (1 if ((len(self.token_ids) - self.context_length - 1) % self.stride > 0) else 0)
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        """
        input_ids = self.token_ids[idx:idx + self.context_length]
        target_ids = self.token_ids[idx+1:idx + self.context_length + 1]
        tensor_input = torch.tensor(input_ids)
        tensor_target = torch.tensor(target_ids)
...
def create_dataloader(
    ...
    data = GPTDataset(token_ids,context_length,stride)
    loader = DataLoader(
        dataset = data,
